...
*Setup instructions for a Gitian build of Foundation Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Foundation
...
- [Building Foundation Core](#building-foundation-core)
...
Clone the git repositories for foundation and Gitian.
...
in the foundation repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Foundation Core
----------------

To build Foundation Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the foundation repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/foundation/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/foundation-project/foundation
...
For example:
```bash
URL=https://github.com/thrasher-/foundation.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit foundation=${COMMIT} --url foundation=${URL} ../foundation/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit foundation=${COMMIT} --url foundation=${URL} ../foundation/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit foundation=${COMMIT} --url foundation=${URL} ../foundation/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the foundation git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../foundation/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/foundation
SIGPATH=/some/root/path/foundation-detached-sigs

./bin/gbuild --url foundation=${BTCPATH},signature=${SIGPATH} ../foundation/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/foundation-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/foundation-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/foundation-osx-build.assert
```
...
[foundation-project/gitian.sigs.FDN](https://github.com/foundation-project/gitian.sigs.FDN/) repository, or if that's not possible create a pull
