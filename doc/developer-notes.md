...
Run with the -testnet option to run with "play foundations" on the test network, if you
...
Foundation Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9454.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9452 for connections and services them.

- BitcoinMiner : Generates foundations (if wallet is enabled).
...
However, in open source software such as Foundation Core, where everyone uses
...
pay attention to for reviewers of Foundation Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Foundation Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Foundation Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Foundation Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:foundation-project/foundation.git
