Foundation Core version *0.15.1* is now available from:

  <https://download.foundation.org/foundation-0.15.1/>
...
A number of changes to the way Foundation Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 virtues per kB (0.00001 FDN). This is to prepare for dropping the minimum transaction fee to 0.1 virtues per kB (0.0001 FDN) in the next release.
